...
d3.json(filename,function(data)
{
	...
	var end_a=sten_a[1]
	var exonanno_a=[[start_a-1,((end_a+1)-(start_a-1))]]; //first exon
	while(true){
		sten_a=(findnextexon(data,end_a,"a"));
		start_a=sten_a[0],end_a=sten_a[1];
		if (end_a===data.length){break;}
	...
	var end_b=sten_b[1]
	var exonanno_b=[[start_b,((end_b+1)-(start_b-1))]]; //first exon, the 2 here is to prevent the removal of duplicate data, i am out of better solution....
	while(true){
		sten_b=(findnextexon(data,end_b,"b"));
		start_b=sten_b[0],end_b=sten_b[1];
		if (end_b===data.length){break;}
...
g.selectAll("rect") //the second one
   		 .data(exonanno_b,function(d){return d;})
    		.enter()
   		 .append("rect")
   		 .attr("x", function(d,i){
   			   return scaling(d[0])+110
	...
	g.selectAll("line")
		...
		.attr("stroke",function(d){
			if (d.a === '-') {return "#C6C7C4"}
            else if (d.a === 'i') {return "#f975b2"}	
            else if (d.a === 'n') {return "transparent"}	
            else if (d.a === 'o') {return "transparent"}
            else { return "#FFE1A8" };
	...
	g.selectAll("line")
		...
		.attr("y2",ay2+20)
		.attr("stroke",function(d){
			if (d.b === '-') {return "#C6C7C4"}
            else if (d.b === 'i') {return "#f975b2"}
            else if (d.b === 'n') {return "transparent"}	
            else if (d.b === 'o') {return "transparent"}		
            else { return "#FFE1A8" };
	...
	g.selectAll("notathing") //plot the border to the front, notathing and cannot possibly overlap is to prevent the data being considered as repeat
   		 .data(exonanno_a,function (d) { return d.id+ " cannot possibly overlap!!"; })
    		.enter()
   		 .append("rect")
   		 .attr("x", function(d,i){
   			   return scaling(d[0])+110
...
g.selectAll("notathing") //the second one
   		 .data(exonanno_b,function (d) { return d.id+ " cannot possibly overlap"; })
    		.enter()
   		 .append("rect")
   		 .attr("x", function(d,i){
   			   return scaling(d[0])+110
